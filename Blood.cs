...
[BepInPlugin("LeeMoriya.Blood", "Blood", "1.18")]
public class BloodMod : BaseUnityPlugin
{
    ...
    public static bool spikeCompat = false;
    ...
    private void RainWorldGame_ctor(On.RainWorldGame.orig_ctor orig, RainWorldGame self, ProcessManager manager)
    {
        ...
        try
        {
            bool regionKitLoaded;
            regionKitLoaded = ModManager.ActiveMods.Any((ModManager.Mod mod) => mod.id == "regionkit");
            if (regionKitLoaded)
            {
                spikeCompat = true;

                Type spikeType = Type.GetType("RegionKit.Modules.Objects.Spike, RegionKit");

                if (spikeType != null)
                {
                    var harmony = new Harmony("bloodmod.spikehook");
                    var original = spikeType.GetMethod("Impaled", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

                    var prefix = typeof(BloodMod).GetMethod(nameof(Prefix), BindingFlags.Static | BindingFlags.NonPublic);
                    harmony.Patch(original, prefix: new HarmonyMethod(prefix));

                    UnityEngine.Debug.Log("Successfully hooked Spike.Impaled.");
                }
                else
                {
                    UnityEngine.Debug.Log("Spike type not found. Hook skipped.");
                }
            }
        }
        catch (Exception ex)
    ...
    private static void Prefix(object __instance, object creature, object chunk)
    {
        BloodEmitter spikeEmitter = new BloodEmitter(null, (chunk as BodyChunk), UnityEngine.Random.Range(0.2f, 2f), UnityEngine.Random.Range(0.5f,4f));
        if ((creature as Creature).room != null)
        {
            (creature as Creature).room.AddObject(spikeEmitter);
        }
    }
    ...
    public static Dictionary<string, Color> defaultColors = new Dictionary<string, Color>()
        {
            ...
            //WATCHER
            { "DrillCrab",          new Color(0.12f, 0.05f, 0.04f) },
            { "Barnacle",           new Color(0.5f, 0.1f, 0f) },
            { "SandGrub",           new Color(0.5f, 0.43f, 0.07f) },
            { "BigSandGrub",        new Color(0.5f, 0.43f, 0.07f) },
            { "BigMoth",            new Color(0.8f, 0.8f, 0.76f) },
            { "SmallMoth",          new Color(0.8f, 0.8f, 0.76f) },
            { "BoxWorm",            new Color(0.5f, 0.43f, 0.07f) },
            { "FireSprite",         new Color(0.8f, 0.5f, 0f) },
            { "Rattler",            new Color(0.3f, 0f, 0.7f) },
            { "SkyWhale",           new Color(0.5f, 0f, 0f) },
            { "ScavengerTemplar",   new Color(0.5f, 0f, 0f) },
            { "ScavengerDisciple",  new Color(0.5f, 0f, 0f) },
            { "Loach",              new Color(0.85f, 0.8f, 0.8f) },
            { "RotLoach",           new Color(0.3f, 0f, 0.7f) },
            { "BlizzardLizard",     new Color(0.8f, 0.8f, 0.8f) },
            { "BasiliskLizard",     new Color(0.3f, 0.2f, 0.1f) },
            { "IndigoLizard",       new Color(0.2f, 0f, 0.8f) },
            { "Rat",                new Color(0.5f, 0f, 0.15f) },
            { "Frog",               new Color(0.5f, 0f, 0.15f) },
            { "Tardigrade",         new Color(0.5f, 0f, 0.15f) },
    ...
    public static Dictionary<string, Color> vibrantColors = new Dictionary<string, Color>()
        {
            ...
            { "Rattler",            new Color(0.3f, 0f, 0.7f) },
            { "SkyWhale",           new Color(0.5f, 0f, 0f) },
            { "ScavengerTemplar",   new Color(0.8f, 0.3f, 0f) },
            { "ScavengerDisciple",  new Color(0.8f, 0.3f, 0f) },
            { "Loach",              new Color(0.85f, 0.8f, 0.8f) },
            { "RotLoach",           new Color(0.3f, 0f, 0.7f) },
            { "BlizzardLizard",     new Color(0.8f, 0.8f, 0.8f) },
            { "BasiliskLizard",     new Color(0.3f, 0.2f, 0.1f) },
